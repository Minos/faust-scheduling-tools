...
detune 		= hslider("detune", 4, 0, 512, 1);
polyphony 	= hslider("polyphony", 1, 0, 4, 1);
output 		= hslider("output volume", 0.5, 0, 1, 0.1);

process =  vgroup("karplus4",
	 		   	vgroup("noise generator", no.noise * hslider("level", 0.5, 0, 1, 0.1))
				 : vgroup("excitator", *(button("play"): trigger(size)))
				<: vgroup("resonator x4", par(i,4, resonator(dur+i*detune, att) * (polyphony > i)))
				:> *(output),*(output)
			);
