...
//-----------------------------------------------
// 				karplus-strong
//		with 16 resonators in parallel
//-----------------------------------------------
...
detune 		= hslider("detune", 16, 0, 512, 1);
polyphony 	= hslider("polyphony", 1, 0, 16, 1);
output 		= hslider("output volume", 0.5, 0, 1, 0.1);

process =  vgroup("karplus16",
	 		   	vgroup("noise generator", no.noise * hslider("level", 0.5, 0, 1, 0.1))
				 : vgroup("excitator", *(button("play"): trigger(size)))
				<: vgroup("resonator x16", par(i,16, resonator(dur+i*detune, att) * (polyphony > i)))
				:> *(output),*(output)
			);
