...
class Test:
    def __init__(self, directory, program_name, strategy):
        ...
        self.path = f"{directory}/{program_name}.plot/" \
                    f"{program_name}_ss{strategy}"
        self.impulse_response = None

    def get_impulse_response(self) -> np.array:
...
def check_strategies(directory, program_name):
    tests = [Test(directory, program_name, s) for s in co.STRATEGIES]
    build_binaries(tests)
...
def main():
    parser = argparse.ArgumentParser(
        prog="Check strategies",
        description="Check that all provided scheduling strategies have the \
    ...
    for file in files:
        ...
        check_strategies(directory, program_name)
